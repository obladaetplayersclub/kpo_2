...
- **Аккаунты (BankAccount):**
  - создание счёта, просмотр баланса
...
- **Импорт / Экспорт (Template Method + Visitor):**
  ...
  - экспорт реализован с помощью паттерна Наблюдатель
...
| **Visitor** (_Наблюдатель_) | `CsvExportVisitor`, `JsonExportVisitor`, `YamlExportVisitor` | Позволяет добавлять новые форматы экспорта без изменения доменных объектов |
